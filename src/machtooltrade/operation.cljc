(ns machtooltrade.operation
  "OperationActor -- one precision-machinery-wholesale operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (MachToolTradeAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Precision
  Machinery Export Governor (:govern) and the rollout phase gate
  ...
  One graph run = one precision-machinery-wholesale operation (intake
  -> advise -> govern -> decide -> commit | hold | approval). No
  unbounded inner loop -- each operation is auditable and checkpointed.
  A machine-order's life is advanced by MANY ops (intake / contract-
  ...
  decision to a human operator (trading supervisor / export-compliance
  officer). The approver resumes with `{:approval {:status :approved}}`
  (or :rejected). `:delivery/dispatch`/`:invoice/settle` ALWAYS reach
  this node when the governor is clean -- see `machtooltrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [machtooltrade.machtooltradeadvisor :as machtooltradeadvisor]
...
(defn build
  ...
  `machtooltrade.store/Store`).
  opts:
    :advisor      -- a `machtooltrade.machtooltradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (machtooltradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MachToolTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (machtooltradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(machtooltradeadvisor/trace request p)]})))

      ;; Precision Machinery Export Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
